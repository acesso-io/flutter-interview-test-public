...
import 'package:flutter_interview_test/domain/use_cases/get_all_characters_with_favorite_use_case.dart';
import 'package:flutter_interview_test/domain/use_cases/set_character_as_favorite_use_case.dart';
...
class _CharacterListState extends BaseState<CharacterListView> {
  ...
  GetAllCharactersWithFavorites getAllCharactersWithFavorites =
      GetAllCharactersWithFavorites.injected();
  SetCharAsFavorite setCharAsFavorite = SetCharAsFavorite.injected();
    ...
    if (state.isBusy) {
      ...
      body = ListView.separated(
        ...
        itemBuilder: (_, int index) => CharacterCardView(
            ...
            onFavoriteClicked: () => _setCharAsFavorite(_characters[index])),
  ...
  void _fetchCharacters() async {
    ...
    final result = await getAllCharactersWithFavorites.run();
    setState(() {
  ...
  void _setCharAsFavorite(Character character) async {
    if (character.id != null) {
      await setCharAsFavorite.run(character.id!, character.favorite ?? false);
      _characters.firstWhere((element) => element.id == character.id).favorite =
